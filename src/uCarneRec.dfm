object fCarneRec: TfCarneRec
  Left = 297
  Top = 105
  Width = 800
  Height = 520
  Caption = 'Carne'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 0
    Top = 0
    Width = 792
    Height = 108
    Align = alTop
    Caption = 'GERAR CARNES  -  Imprimir o que foi gerado em Gerar Titulos'
    Color = clMoneyGreen
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 0
    object Label5: TLabel
      Left = 116
      Top = 61
      Width = 19
      Height = 16
      Alignment = taRightJustify
      Caption = 'at'#233
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label7: TLabel
      Left = 59
      Top = 61
      Width = 46
      Height = 16
      Caption = 'Insc. de'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label2: TLabel
      Left = 120
      Top = 16
      Width = 71
      Height = 16
      Caption = 'Vencimento'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label1: TLabel
      Left = 5
      Top = 16
      Width = 53
      Height = 16
      Caption = 'Emiss'#227'o'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object Label8: TLabel
      Left = 174
      Top = 61
      Width = 35
      Height = 16
      Caption = 'S'#243'cio'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 235
      Top = 16
      Width = 57
      Height = 16
      Caption = 'Cobrador'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel
      Left = 4
      Top = 61
      Width = 37
      Height = 16
      Caption = 'Grupo'
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
    object DBEdit2: TDBEdit
      Left = 512
      Top = 80
      Width = 65
      Height = 21
      DataField = 'SOC'
      DataSource = dsCarne
      TabOrder = 9
    end
    object edCodCliente: TEdit
      Left = 232
      Top = 78
      Width = 449
      Height = 24
      TabStop = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 3
    end
    object DateEdit2: TDateEdit
      Left = 120
      Top = 33
      Width = 112
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
    end
    object DateEdit1: TDateEdit
      Left = 5
      Top = 33
      Width = 113
      Height = 24
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
    end
    object cbCobrador: TJvDBSearchComboBox
      Left = 234
      Top = 33
      Width = 516
      Height = 24
      DataField = 'NOME'
      DataSource = DtSrc
      BevelKind = bkFlat
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemHeight = 16
      ParentFont = False
      TabOrder = 2
    end
    object BitBtn5: TBitBtn
      Left = 685
      Top = 65
      Width = 41
      Height = 38
      TabOrder = 4
      OnClick = BitBtn5Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C00000}
    end
    object Edit2: TEdit
      Left = 4
      Top = 78
      Width = 52
      Height = 24
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
    end
    object Edit4: TEdit
      Left = 174
      Top = 78
      Width = 56
      Height = 24
      TabStop = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      CharCase = ecUpperCase
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ReadOnly = True
      TabOrder = 6
    end
    object Edit3: TJvValidateEdit
      Left = 57
      Top = 78
      Width = 57
      Height = 24
      Flat = False
      ParentFlat = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      CriticalPoints.MaxValueIncluded = False
      CriticalPoints.MinValueIncluded = False
      EditText = '1'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 7
    end
    object Edit1: TJvValidateEdit
      Left = 115
      Top = 78
      Width = 57
      Height = 24
      Flat = False
      ParentFlat = False
      BevelKind = bkFlat
      BorderStyle = bsNone
      CriticalPoints.MaxValueIncluded = False
      CriticalPoints.MinValueIncluded = False
      EditText = '500'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
    end
    object BitBtn1: TBitBtn
      Left = 736
      Top = 72
      Width = 27
      Height = 25
      Caption = 'X'
      TabOrder = 10
      OnClick = BitBtn1Click
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 444
    Width = 792
    Height = 49
    Align = alBottom
    Color = clMoneyGreen
    TabOrder = 1
    object Label3: TLabel
      Left = 672
      Top = 16
      Width = 9
      Height = 13
      Caption = '...'
    end
    object BitBtn3: TBitBtn
      Left = 383
      Top = 7
      Width = 95
      Height = 36
      Caption = '&Imprimir'
      TabOrder = 1
      OnClick = BitBtn3Click
      Glyph.Data = {
        
        C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0C0}
    end
    object BitBtn4: TBitBtn
      Left = 516
      Top = 7
      Width = 95
      Height = 36
      Caption = '&Sair'
      TabOrder = 2
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBF00}
    end
    object BitBtn6: TBitBtn
      Left = 250
      Top = 7
      Width = 95
      Height = 36
      Caption = '&Pesquisar'
      TabOrder = 0
      OnClick = BitBtn6Click
      Glyph.Data = {
        
        BFBFBFBF0000}
    end
  end
  object DBGrid1: TDBGrid
    Left = 5
    Top = 110
    Width = 756
    Height = 323
    DataSource = dsCarne
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'PARCELAS'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'STATUS'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SOMA'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NOME_SOCIO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'SOC'
        Visible = True
      end>
  end
  object sdsCobrador: TSQLDataSet
    CommandText = 'select NOME, ID_COB from COBRANCA'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.SQLObitos
    Left = 96
    Top = 184
    object sdsCobradorNOME: TStringField
      FieldName = 'NOME'
      Size = 80
    end
    object sdsCobradorID_COB: TIntegerField
      FieldName = 'ID_COB'
      Required = True
    end
  end
  object XPMenu1: TXPMenu
    DimLevel = 30
    GrayLevel = 10
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMenuText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Color = clBtnFace
    DrawMenuBar = False
    IconBackColor = clBtnFace
    MenuBarColor = clBtnFace
    SelectColor = clHighlight
    SelectBorderColor = clHighlight
    SelectFontColor = clMenuText
    DisabledColor = clInactiveCaption
    SeparatorColor = clBtnFace
    CheckedColor = clHighlight
    IconWidth = 24
    DrawSelect = True
    UseSystemColors = True
    UseDimColor = False
    OverrideOwnerDraw = False
    Gradient = False
    FlatMenu = False
    AutoDetect = True
    Active = True
    Left = 480
    Top = 160
  end
  object dspCobrador: TDataSetProvider
    DataSet = sdsCobrador
    Left = 125
    Top = 184
  end
  object cdsCobrador: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspCobrador'
    Left = 154
    Top = 184
    object cdsCobradorNOME: TStringField
      FieldName = 'NOME'
      Size = 80
    end
    object cdsCobradorID_COB: TIntegerField
      FieldName = 'ID_COB'
      Required = True
    end
  end
  object DtSrc: TDataSource
    DataSet = cdsCobrador
    Enabled = False
    Left = 183
    Top = 184
  end
  object dtsCR: TDataSource
    DataSet = cdsGerar
    Left = 184
    Top = 246
  end
  object sdsGerar: TSQLDataSet
    CommandText = 
      'SELECT rc.id_recebimentos'#13#10'            , RC.STATUS'#13#10'            ' +
      ', RC.EMISSAO'#13#10'            , RC.VENCIMENTO'#13#10'            , RC.valo' +
      'r_nf'#13#10'            , RC.LOTE'#13#10'            , SC.GRUPO'#13#10'           ' +
      ' , SC.NOME_SOCIO'#13#10'            , SC.N_INSCRICAO'#13#10'            , SC' +
      '.CPF_CGC'#13#10'            , cb.NOME'#13#10'            , rc.MESANO'#13#10'      ' +
      '      , rc.n_boleto'#13#10'            , rc.titulo '#13#10'            , rc.' +
      'ID_SOCIO'#13#10'            , rc.ID   '#13#10'FROM recebimentos RC'#13#10'     LEF' +
      'T OUTER JOIN socios SC ON SC.id_socio = RC.id_socio'#13#10'     LEFT O' +
      'UTER JOIN cobranca CB ON CB.id_cob = SC.id_cob '#13#10'WHERE rc.MESANO' +
      ' = :gr '#13#10'       and  sc.GRUPO = :gp '#13#10'       and  sc.N_INSCRICAO' +
      ' between :insc and :insc1 '#13#10'       and ((sc.ID_COB = :cb) or (:c' +
      'b = 9999999))'#13#10'             order by SC.N_INSCRICAO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'gr'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'gp'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc1'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cb'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cb'
        ParamType = ptInput
      end>
    SQLConnection = DM.SQLObitos
    Left = 96
    Top = 246
    object sdsGerarID_RECEBIMENTOS: TIntegerField
      FieldName = 'ID_RECEBIMENTOS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object sdsGerarSTATUS: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Size = 30
    end
    object sdsGerarEMISSAO: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarVENCIMENTO: TDateField
      FieldName = 'VENCIMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarVALOR_NF: TFloatField
      FieldName = 'VALOR_NF'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarLOTE: TIntegerField
      FieldName = 'LOTE'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarGRUPO: TStringField
      FieldName = 'GRUPO'
      ProviderFlags = []
      FixedChar = True
      Size = 1
    end
    object sdsGerarNOME_SOCIO: TStringField
      FieldName = 'NOME_SOCIO'
      ProviderFlags = []
      Size = 80
    end
    object sdsGerarN_INSCRICAO: TIntegerField
      FieldName = 'N_INSCRICAO'
      ProviderFlags = []
    end
    object sdsGerarCPF_CGC: TStringField
      FieldName = 'CPF_CGC'
      ProviderFlags = [pfInUpdate]
      Size = 18
    end
    object sdsGerarNOME: TStringField
      FieldName = 'NOME'
      ProviderFlags = []
      Size = 80
    end
    object sdsGerarN_BOLETO: TStringField
      FieldName = 'N_BOLETO'
      ProviderFlags = [pfInUpdate]
      Size = 30
    end
    object sdsGerarTITULO: TIntegerField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarID_SOCIO: TIntegerField
      FieldName = 'ID_SOCIO'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarID: TIntegerField
      FieldName = 'ID'
      ProviderFlags = [pfInUpdate]
    end
    object sdsGerarMESANO: TStringField
      FieldName = 'MESANO'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
  end
  object dspGerar: TDataSetProvider
    DataSet = sdsGerar
    UpdateMode = upWhereKeyOnly
    Left = 125
    Top = 246
  end
  object cdsGerar: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'gr'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'gp'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc1'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cb'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cb'
        ParamType = ptInput
      end>
    ProviderName = 'dspGerar'
    Left = 154
    Top = 246
    object cdsGerarID_RECEBIMENTOS: TIntegerField
      FieldName = 'ID_RECEBIMENTOS'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object cdsGerarSTATUS: TStringField
      FieldName = 'STATUS'
      ProviderFlags = [pfInUpdate]
      Size = 30
    end
    object cdsGerarEMISSAO: TDateField
      FieldName = 'EMISSAO'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarVENCIMENTO: TDateField
      FieldName = 'VENCIMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarVALOR_NF: TFloatField
      FieldName = 'VALOR_NF'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarLOTE: TIntegerField
      FieldName = 'LOTE'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarGRUPO: TStringField
      FieldName = 'GRUPO'
      ProviderFlags = []
      FixedChar = True
      Size = 1
    end
    object cdsGerarNOME_SOCIO: TStringField
      FieldName = 'NOME_SOCIO'
      ProviderFlags = []
      Size = 80
    end
    object cdsGerarN_INSCRICAO: TIntegerField
      FieldName = 'N_INSCRICAO'
      ProviderFlags = []
    end
    object cdsGerarCPF_CGC: TStringField
      FieldName = 'CPF_CGC'
      ProviderFlags = [pfInUpdate]
      Size = 18
    end
    object cdsGerarNOME: TStringField
      FieldName = 'NOME'
      ProviderFlags = []
      Size = 80
    end
    object cdsGerarN_BOLETO: TStringField
      FieldName = 'N_BOLETO'
      ProviderFlags = [pfInUpdate]
      Size = 30
    end
    object cdsGerarTITULO: TIntegerField
      FieldName = 'TITULO'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarID_SOCIO: TIntegerField
      FieldName = 'ID_SOCIO'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarID: TIntegerField
      FieldName = 'ID'
      ProviderFlags = [pfInUpdate]
    end
    object cdsGerarMESANO: TStringField
      FieldName = 'MESANO'
      ProviderFlags = [pfInUpdate]
      Size = 10
    end
  end
  object VCLReport1: TVCLReport
    AsyncExecution = False
    Title = 'Untitled'
    Left = 328
    Top = 184
  end
  object sqlCarne: TSQLDataSet
    CommandText = 
      'select * from carne_imprimir( :gr, :data1 , :data2, :insc , :ins' +
      'c1)'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'gr'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'data1'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'data2'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc1'
        ParamType = ptInput
      end>
    SQLConnection = DM.SQLObitos
    Left = 486
    Top = 280
    object sqlCarnePARCELAS: TIntegerField
      FieldName = 'PARCELAS'
    end
    object sqlCarneSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 30
    end
    object sqlCarneSOMA: TFloatField
      FieldName = 'SOMA'
    end
    object sqlCarneNOME_SOCIO: TStringField
      FieldName = 'NOME_SOCIO'
      Size = 80
    end
    object sqlCarneSOC: TIntegerField
      FieldName = 'SOC'
    end
  end
  object dspCarne: TDataSetProvider
    DataSet = sqlCarne
    Options = [poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 534
    Top = 280
  end
  object cdsCarne: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'gr'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'data1'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'data2'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'insc1'
        ParamType = ptInput
      end>
    ProviderName = 'dspCarne'
    Left = 582
    Top = 280
    object cdsCarnePARCELAS: TIntegerField
      FieldName = 'PARCELAS'
    end
    object cdsCarneSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 30
    end
    object cdsCarneSOMA: TFloatField
      FieldName = 'SOMA'
    end
    object cdsCarneNOME_SOCIO: TStringField
      FieldName = 'NOME_SOCIO'
      Size = 80
    end
    object cdsCarneSOC: TIntegerField
      FieldName = 'SOC'
    end
  end
  object dsCarne: TDataSource
    DataSet = cdsCarne
    Left = 646
    Top = 280
  end
end

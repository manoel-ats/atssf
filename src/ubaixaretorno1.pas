...
var
  fBaixaretorno1: TfBaixaretorno1;

implementation

uses UDM, uArquivoRetorno1  ;

{$R *.dfm}

procedure TfBaixaretorno1.FormShow(Sender: TObject);
begin
  if cds.Active then  
    cds.Close;
...
procedure TfBaixaretorno1.BitBtn2Click(Sender: TObject);
begin
  close;
end;

procedure TfBaixaretorno1.BitBtn1Click(Sender: TObject);
begin
...
//     begin
        ...
        begin
          ...
          cdsNOMEARQUIVORETORNO.AsString := ExtractFileName(fArquivoRetorno1.OpenDialog1.FileName);
          cds.ApplyUpdates(0);
          v_pago := v_resto;
          ...
          //atualizo o memo
          texto_memo := '';
          texto_memo := texto_memo + 'COBST : ';

          if(fArquivoRetorno1.de = 'baixa1') then
          begin
            texto_memo := 'Título nº: ' + IntToStr(fArquivoRetorno1.c_retornoTITULO.AsInteger) + ' Data: ' +
            fArquivoRetorno1.c_retornoDATA.AsString + ' valor R$ : ' +
            FloatToStr(fArquivoRetorno1.c_retornoVALOR.AsFloat);
          end;

          if(fArquivoRetorno1.de = 'baixa') then
          begin
            texto_memo := 'Título nº: ' + IntToStr(fArquivoRetorno1.cdsTITULO.AsInteger) + ' Data: ' +
            fArquivoRetorno1.cdsDATA.AsString + ' valor R$ : ' +
            FloatToStr(fArquivoRetorno1.cdsVALOR.AsFloat);
          end;
          ...
          fArquivoRetorno1.Memo1.Lines.Add(texto_memo);

...
type
  TfRemessaBanco = class(TForm)
    ...
    sds_CRCPF_CGC: TStringField;
    cds_CRCPF_CGC: TStringField;
...
procedure TfRemessaBanco.SpeedButton1Click(Sender: TObject);
var caminho : string;
begin
  ...
  if (not cdsBanco.Active) then
   ...
   if Edit8.Text = 'BANCO SANTANDER' then
     Edit5.Text := caminho + '_tri' + Edit2.Text + '_' + FormatDateTime('dd_mm_yy', now) + '.txt';
...
procedure TfRemessaBanco.BitBtn4Click(Sender: TObject);
begin
  SaveDialog1.Execute;
  Edit5.Text := SaveDialog1.FileName;
end;

procedure TfRemessaBanco.BitBtn2Click(Sender: TObject);
begin
  if Edit8.Text = 'BRASIL' then
  begin
    remessaBB;
...
procedure TfRemessaBanco.remessaBB;
var
...
procedure TfRemessaBanco.SpeedButton2Click(Sender: TObject);
begin
...
procedure TfRemessaBanco.BitBtn1Click(Sender: TObject);
begin
 if Edit2.Text = '' then
 begin
 ...
 if cdsCR.Active then
  ...
  //cdsCR.Params[5].Clear;

  cdsCR.Params[0].Value := StrToDate(DateEdit2.Text);
  //cdsCR.Params[1].Value := StrToDate(DateEdit2.Text);
 ...
 if Edit4.Text <> '' then
  ...
  label4.Caption := IntToStr(cdsCR.RecordCount);

end;

procedure TfRemessaBanco.dxButton5Click(Sender: TObject);
begin
  close;
end;

procedure TfRemessaBanco.remessaSANTANDER;
var
   ACedenteTipoInscricao, ASacadoTipoInscricao, aTitulosTipoOcorrencia,
   aTitulosEspecieDocumento,aTitulosAceite, NomArquivo, Registro : string;
   cTotalTitulos: Currency;

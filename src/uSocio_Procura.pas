...
begin
    //***************************************************************************
    if (varonde = 0) then //abri o Procura pelo form fSocios (Cadastro de Sócios)
        varform0;

    if (varonde = 1) then //abri o Procura pelo form PRINCIPAL
        varform1;

    if (varonde = 2) then //estou vindo do cadastro de Óbitos
        varform2;
...
begin
      ...
      abreTabelas;//Vejo se o Socio está em carência, abro tabelas dependentes, endereço etc...
end;
...
if dm.cds_Obitos.State in [dsBrowse] then
         dm.cds_Obitos.Edit;
       // se for sócio busco o endereço..
       if (cds_procDESCRICAO.AsString = 'SOCIO') then
       begin
         // Abro o endereço do sócio
  ...
  //     dm.cds_ObitosDATA_NASC.Value:=cds_procDTNASC.Value;
       ...
       begin
          if cds_procDESCRICAO.AsString = 'SOCIO' then
            MessageDlg('Sócio em Carência', mtWarning, [mbOK], 0);
          if cds_procDESCRICAO.AsString <> 'SOCIO' then
            MessageDlg('Dependente em Carência', mtWarning, [mbOK], 0);
       end;
       cds_proc.Close;
       Close;
end;
...
begin
       {------------- aqui vejo se o Sócio está em carência -----------------}
          ...
          if dif < dm.cds_socioCARENCIA.AsInteger then
             fSocios.Edit4.Text := 'Sócio em Carência'
          else
             fSocios.Edit4.Text := '';

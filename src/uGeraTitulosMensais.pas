  ...
  { NomArquivo1 := 'C:\BancoBrasil\LISTA' + Edit2.Text + '_' + FormatDateTime('dd_mm_yy', now) + '.txt' ;
   ...
   begin
    ...
    {if (TotalPendente.Active) then
     ...
     Registro := Registro + Formatar(cdsGerarNOME_SOCIO.AsString,40,true,' '); // NOME DO SACADO  19 a 78 {O Certo é 60 }
     ...
     Registro := Registro + Formatar(sqlEnderecoENDERECO.AsString + '-' + sqlEnderecoBAIRRO.AsString, 60); // ENDEREÇO DO SACADO 79 a 138
  ...
  mesano := mesano + IntToStr(ano);
   ...
   begin
     ...
     begin
       ...
       {** Altero o Número do Título na tabela recebimentos}
       Label8.Caption := '';
       Label8.Caption := 'Atualizando Nº dos Recibos...';
       Refresh;
       FlatGauge1.MaxValue := cdsGerar.RecordCount;
       contador := StrToInt(Edit9.Text);
       cdsGerar.First;
       while not cdsGerar.Eof do
        ...
        FlatGauge1.Progress := 0;
        Label8.Caption := '';
        Label8.Caption := 'Nº dos Recibos atualizados com SUCESSO...';
        Refresh;
   end;
...
begin
   ...
   if not cdsGerar.Active then
   begin
      MessageDlg('Antes de gerar Remessa, É preciso escolher o Cobrador e fazer a Pesquisa para Gerar o Arquivo.', mtWarning, [mbOK], 0);
      exit;
   end;
   {** verifico se foi selecionado o banco e se ele existe na tabela BANCOS}
   if cdsBanco.Active then
   ...
   Rewrite(arquivo);
    ...
    FlatGauge1.MaxValue := cdsGerar.RecordCount;
     ...
     begin
       ...
       // GERAR REGISTRO DETALHE  - SEGMENTO P        TIPO 3
           ...
           Registro := Registro + Formatar(IntToStr(cdsGerarTITULO.AsInteger) ,12,false,'0');// 045 –057	Identificação do título no Banco  	N	013		Nosso Número 	15
           Registro := Registro + Formatar(IntToStr(cdsGerarTITULO.AsInteger) + Modulo11(IntToStr(cdsGerarTITULO.AsInteger)),1,false,'0');// // 045 –057	Identificação do título no Banco  	N	013		Nosso Número 	15
    ...
    //     Registro := Registro + '1';// 059 - 059	Forma de Cadastramento 	N	001			6   1 com registro 2 sem registro
           ...
           Registro := Registro + Formatar(IntToStr(cdsGerarTITULO.AsInteger) ,15,false,'0');  // 063 - 077	Nº  do documento 	A	015		Seu número	16
           ...
           Registro := Registro + FormatCurr('000000000000000',cdsGerarVALOR_NF.AsFloat * 100);// 086 - 100	Valor nominal do título	N	015	2	Decimais sem separador	18
         ...
         //GERAR REGISTRO DETALHE - SEGMENTO Q    TIPO 3
           ...
           Registro := Registro + Formatar(cdsGerarRUA.AsString, 40); // 074 - 113	Endereço  sacado	A	040
           ...
           Registro := Registro + Formatar(cdsGerarUF.AsString,2,false); // 152 - 153	Unidade da federação do sacado	A	002

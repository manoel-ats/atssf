...
uses
  ...
  Mask, SqlExpr, Provider, DB, DBClient, ExtCtrls, DBCtrls,
  ToolEdit, DBXpress;
...
uses uProcGrupo, UDM, uProcurar, URemessaBanco1, uPrincipal;
...
begin
 ...
 begin
  ...
  if MessageDlg('Deseja realmente excluir este Lançamento?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
  begin
      TD.TransactionID := 1;
      ...
      deletar := deletar + QuotedStr(varUsuario);
      deletar := deletar + ' WHERE ID_RECEBIMENTOS = ';
      deletar :=  deletar + IntToStr(cds_CRID_RECEBIMENTOS.AsInteger);

      deletar := deletar + ' and ID_SOCIO = ';
      deletar := deletar + IntToStr(cds_CRID_SOCIO.AsInteger);

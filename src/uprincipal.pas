...
begin
  ...
  lbl2.Caption := ' ATS - SCF  versão: ' + GetVersion;
  Label3.Caption := ' ATS - SCF  versão: ' + GetVersion;
  TD.TransactionID := 1;
...
begin
  ...
  begin
   MessageDlg('Usuário sem acesso a esse item de menu', mtConfirmation, [mbOK], 0);
   exit;
  end;
...
begin
 ...
 //   Image1.Picture.LoadFromFile(fTemas.temaCAMINHO.AsString);
  ...
  if (Dm.mensagemInicial = 'A Licença de uso expirada.') then
  begin
    lblMensagemSistema.Caption := 'A Licença de uso expirada.';
    //if (Dm.mensagemInicial = 'A Licença de uso expirada.') then
 ...
 //   lblMensagemSistema.Caption := 'A Licença de uso expirada.';
  ...
  //if (liberado <> '') then
  if (liberado = 'A Licença de uso expirada..') then
  begin
    MessageDlg('Licença do uso expirada, entre em contato com a ATS .', mtWarning, [mbOK], 0);
    lblMensagemSistema.Caption := 'A Licença de uso expirada..';
    SpeedButton1.Enabled := False;
...
begin
    ...
    try
     ...
     fGeraTitulos.GroupBox1.Caption := 'GERAR TÍTULOS - MENSAL -  GERAR MES a MES ';
...
begin
  ...
  try
    ...
    fMov.Label9.Caption := 'Com Vencimento até o Dia';
    fMov.ShowModal;
  finally
    fMov.Free;
  end;
...
begin
    ...
    try
     ...
     fGeraTitulos.GroupBox1.Caption := 'GERAR TÍTULOS - TRIMESTRAL -  GERAR MES a MES ';
     fGeraTitulos.ShowModal;
    finally
     fGeraTitulos.Free;
     varonde := 0;

...
begin
  ...
  if dm.cdsPermissao.State in [dsInsert] then
     dm.cdsPermissao.ApplyUpdates(0);
  if dm.cdsPermissao.State in [dsEdit] then
  begin
    TD.TransactionID := 1;
  TD.IsolationLevel := xilREADCOMMITTED;
  dm.SQLObitos.StartTransaction(TD);
  atualizar := '';
  atualizar := 'update PERMISSAO set INCLUIR = ' + QuotedStr(dm.cdsPermissaoINCLUIR.Text);
  atualizar := atualizar + ' , EXCLUIR = ' + QuotedStr(dm.cdsPermissaoEXCLUIR.Text);
  atualizar := atualizar + ' , ALTERAR = ' + QuotedStr(dm.cdsPermissaoALTERAR.Text);
  atualizar := atualizar + ' , CONSULTAR = ' + QuotedStr(dm.cdsPermissaoCONSULTAR.Text);
  atualizar := atualizar + ' where TABELA = ';
  atualizar := atualizar + QuotedStr(Label3.Caption);
  atualizar := atualizar + ' and LOGIN = ';
  atualizar := atualizar + QuotedStr(DM.cds_SenhaLOGIN.AsString);
...
begin
  ...
  dm.SQLObitos.StartTransaction(TD);
  atualizar := '';
  atualizar := 'update PERMISSAO set CONSULTAR = ' + QuotedStr(dm.cdsPermissaoCONSULTAR.Text);
  atualizar := atualizar + ' where TABELA = ';
  atualizar := atualizar + QuotedStr(Label3.Caption);
  atualizar := atualizar + ' and LOGIN = ';
  atualizar := atualizar + QuotedStr(DM.cds_SenhaLOGIN.AsString);
...
begin
  ...
  Label3.Caption := 'Receber Mensalidades';
end;
...
begin
  ...
  Label3.Caption := 'Sócios Cancelados';
end;
...
begin
  ...
  Label3.Caption := 'Rel. Carteirinha';
end;
...
procedure TfGerencia_Acessos.EstadoCivil1Click(Sender: TObject);
begin
  if dm.cdsPermissao.Active then
    dm.cdsPermissao.Close;
  dm.cdsPermissao.Params[0].AsString := DM.cds_SenhaLOGIN.AsString;
  ...
  begin
   dm.cdsPermissao.Append;
   dm.cdsPermissaoLOGIN.AsString := DM.cds_SenhaLOGIN.AsString;
   dm.cdsPermissaoTABELA.AsString := 'estcivil';
   dm.cdsPermissaoCHAVE.AsString :=
     DM.cds_SenhaLOGIN.AsString + 'estcivil';
  end;

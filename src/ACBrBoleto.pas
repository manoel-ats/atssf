...
Uses ACBrUtil, ACBrBancoBradesco, ACBrBancoBrasil, ACBrBancoItau, ACBrBancoSicredi,
     ACBrCaixaEconomica,  ACBrBancoSantander,
     ACBrBancoHSBC,Forms,  uGeraArquivo ,
     {$IFDEF COMPILER6_UP} StrUtils {$ELSE} ACBrD5 {$ENDIF}, Math;    //ACBrBancoBanrisul,, ACBrBanestes
...
begin
  ...
  if(fGeraArquivo.gera_carne <> 'CA') then
...
begin
   if (fGeraArquivo.gera_carne <> 'CA') then   // campo tabela EMPRESA ESPECIEDOCUMENTO
   begin
   if (Cedente.Nome= '') or (cedente.Conta = '') or (Cedente.ContaDigito ='') or
      (Cedente.Agencia = '') or (Cedente.AgenciaDigito = '')
    then

CREATE OR ALTER PROCEDURE TITULOS_CARNE (
    gp char(1),
    insc integer,
    insc1 integer,
    emi date)
...
for SELECT rc.id_recebimentos, RC.STATUS, RC.EMISSAO, RC.VENCIMENTO, RC.valor_nf
            ...
            , rc.MESANO, rc.n_boleto, rc.titulo, rc.ID_SOCIO, SC.id_cob, SC.TIPO_DOC
    FROM recebimentos RC
       ...
       WHERE ((sc.GRUPO = :gp) or (:gp = 't'))  and (sc.N_INSCRICAO between :insc and :insc1)
           and (rc.TIPO_DOC = 'CA') and (rc.STATUS = 'Pendente') and rc.situacao = 'Ativo'
           and rc.EMISSAO = :emi
     into :id_recebimentos, :status, :emissao, :vencimento, :valor_nf,
          :lote, :grupo, :nome_socio, :n_inscricao, :cpf_cgc,
          :mesano, :n_boleto, :titulo, :id_socio, :id , :TIPO_DOC
     do
     begin

...
DECLARE VARIABLE FAIXAPGTO INTEGER;
DECLARE VARIABLE VALORPARCELA double precision;
begin
    -- pego dia de pagamento e faixa do SOCIO
    select s.DIAPARAPGTO, s.FAIXA from SOCIOS s
     where s.ID_SOCIO = :ID_SOCIO
    into :DIA_B, :FAIXAPGTO; 

    select v.VALOR from VALORES v
     where v.FAIXA = :FAIXAPGTO
    into :VALORPARCELA; 
   ...
   /* ------------------------------------------------------------------------- */
    ...
    -- T = (:tit_banco + :I);
     ...
     VALUES
       (:id_socio, :VALORPARCELA, :VALORPARCELA, :VALORPARCELA, :h,
...
GRANT EXECUTE
 ON PROCEDURE SP_CARNE TO  SYSDBA;
